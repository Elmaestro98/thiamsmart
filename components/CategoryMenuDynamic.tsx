...
export default function CategoryMenu({
  ...
  if (loading) {
    return (
      <div
        className={cn(
          "flex items-center gap-2 bg-shop_light_bg text-white px-5 py-3 font-bold text-sm",
          mobile ? "rounded-lg w-full" : "rounded-t-sm",
        )}
      >
  ...
  return (
    <Menu as="div" className={cn("relative", mobile && "w-full")}>
      {({ open }) => (
        <>
          <MenuButton
            className={cn(
              "flex items-center gap-2 bg-shop_light_bg text-white px-5 py-3 font-bold text-sm hover:bg-opacity-90 transition-all duration-150",
              mobile
          ...
          >
            <span className="flex items-center gap-2">
              <LayoutGrid className="text-shop_light_brown" size={18} />
              <span className="text-shop_light_brown">
                Toutes les catégories
              </span>
            </span>
            <ChevronDown
              className={cn(
                "text-shop_light_brown transition-transform duration-200",
                open && "rotate-180",
              )}
              size={15}
            />
          ...
          >
            <MenuItems
              className={cn(
                "absolute left-0 top-full mt-2 bg-white shadow-2xl z-50 focus:outline-none overflow-hidden border border-gray-100 max-h-96 overflow-y-auto",
                mobile ? "w-full rounded-xl" : "w-[85vw] max-w-80 rounded-b-xl",
              )}
            >
              {categories.length > 0 ? (
                categories.map((cat) => (
